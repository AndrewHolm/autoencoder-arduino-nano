...
#include "encoder.h" // This is the header file containing the model's data (weights and biases)
#include <vector>
#include <cmath>
...
void loop() {
  ...
  while(samples_num < NUM_TEST_SAMPLES){
   ...
   normalizeData(samples);
    ...
    // get the output for condensed data (encoded)
    for (int i = 0; i < LATENT_DIM; i++) {
      Serial.print(tflu_o_tensor->data.f[i]);
      Serial.print(", ");
    }
    
    Serial.println(" ");
    Serial.println("--------End encoded data-------- ");
